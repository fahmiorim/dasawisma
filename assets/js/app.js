...
function _init() {
  ...
  $.AdminLTE.tree = function (menu) {
    ...
    function highlightActiveMenu() {
      ...
      if (currentModule) {
        // Find the link that matches the current module (exact match)
        $(menu + " li a").each(function () {
          var $this = $(this);
          var href = $this.attr("href");

          if (href) {
            // Extract module from href
            var hrefMatch = href.match(/[?&]module=([^&]+)/);
            if (hrefMatch && hrefMatch[1] === currentModule) {
              // Add active class to the parent li
              $this.parent("li").addClass("active");
              // Add inline style to ensure visual highlight
              $this.css("background-color", "#1e282c");
              $this.css("color", "#fff");
              // If this link is inside a treeview-menu, also activate the parent treeview
              if ($this.parent().parent().hasClass("treeview-menu")) {
                var parentTreeview = $this.parents("li.treeview").first();
