...
if (empty($_SESSION['ses_user']) AND empty($_SESSION['ses_password'])){  
  ...
  switch($act){
	...
	if ($_SESSION['ses_level'] == 'admin' || $_SESSION['ses_level'] == 'admpkk' || $_SESSION['ses_level'] == 'admkec') {
		$count_query = pg_query($koneksi, "SELECT COUNT(*) as total FROM sipkegiatan");
		$count_result = pg_fetch_array($count_query);
		$count = $count_result['total'];
		$query_data = "SELECT * FROM sipkegiatan ORDER BY tahun DESC, nobln DESC LIMIT $batas OFFSET $posisi";
	} else {
		$kodekel = pg_escape_string($koneksi, $_SESSION['ses_kodekel']);
		$count_query = pg_query($koneksi, "SELECT COUNT(*) as total FROM sipkegiatan WHERE kodekel='$kodekel'");
		$count_result = pg_fetch_array($count_query);
		$count = $count_result['total'];
		$query_data = "SELECT * FROM sipkegiatan WHERE kodekel='$kodekel' ORDER BY tahun DESC, nobln DESC LIMIT $batas OFFSET $posisi";
	}
			...
			?>
                      <tr>
						...
						<td style='text-align:center'>
							<a href="?module=lihatsipkegiatan&id=<?php echo $lk['id']; ?>" class="btn bg-orange btn-flat margin" data-toggle="tooltip" data-placement="top" title="Lihat"><i class="fa fa-desktop"></i></a>
							<a href="?module=editsipkegiatan&id=<?php echo $lk['id']; ?>" class="btn bg-olive btn-flat margin" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fa fa-edit"></i></a>
							<a href="?module=hapussipkegiatan&id=<?php echo $lk['id']; ?>" class="btn btn-danger btn-flat margin" data-toggle="tooltip" data-placement="top" title="Hapus" onclick="return confirm('Yakin ingin menghapus data ini?')"><i class="fa fa-remove"></i></a>
						</td>
                      </tr>
					  <?php
                $no++;
						...
						if ($jml_hal > 1) {
							for ($i = 1; $i <= $jml_hal; $i++) {
								if ($i == $hal) {
									...
									echo "<li><a href='?module=sipkegiatan&hal=$i'>$i</a></li>";
								}
							}
						}
						?>
