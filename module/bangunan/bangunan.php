...
if (empty($_SESSION['ses_user']) and empty($_SESSION['ses_password'])) {
	...
	$aksi = "module/bangunan/aksi_bangunan.php";
	...
	switch ($act) {
		default:
			...
			if ($_SESSION['ses_level'] == 'admin' || $_SESSION['ses_level'] == 'admpkk' || $_SESSION['ses_level'] == 'admkec') {
				$count_query = pg_query($koneksi, "SELECT COUNT(*) as total FROM bangunan");
				$title = "DATA BANGUNAN PKK";
				$query_data = "SELECT * FROM bangunan ORDER BY id DESC LIMIT $batas OFFSET $posisi";
			} else {
				$kodekel = pg_escape_string($koneksi, $_SESSION['ses_kodekel']);
				$count_query = pg_query($koneksi, "SELECT COUNT(*) as total FROM bangunan WHERE kodekel='$kodekel'");
				$title = "DATA BANGUNAN PKK DESA " . $_SESSION['ses_namakel'];
				$query_data = "SELECT * FROM bangunan WHERE kodekel='$kodekel' ORDER BY id DESC LIMIT $batas OFFSET $posisi";
			}
			...
			?>
				...
				<div class='box-body'>
					<div style="text-align:right">
						...
						<a  class="btn bg-blue margin" data-toggle="tooltip" data-placement="top" title="Print Laporan" href="?module=lapbangunan" target="_blank"><i class="fa fa-print"></i> Print Laporan</a>
						<a class="btn bg-purple margin" href="?module=bangunan&act=tambahbangunan"><i class="fa fa-plus"></i> Tambah</a>
					</div>
								...
								if (pg_num_rows($lingk) > 0) {
									while ($lk = pg_fetch_array($lingk)) {
										echo "<tr>
											...
											<td>{$lk['kecamatan']}</td>
											<td>
												<a href='?module=lihatbangunan&id={$lk['id']}' class='btn btn-xs btn-info'><i class='fa fa-eye'></i></a>
												<a href='?module=editbangunan&id={$lk['id']}' class='btn btn-xs btn-warning'><i class='fa fa-edit'></i></a>
												<a href='?module=hapusbangunan&id={$lk['id']}' class='btn btn-xs btn-danger' onclick=\"return confirm('Yakin ingin menghapus data ini?')\"><i class='fa fa-trash'></i></a>
											</td>
										</tr>";
										$no++;
									}
				...
				<div class="box-footer clearfix">
					<ul class="pagination pagination-sm no-margin pull-right">
						...
						if ($hal > 1) {
							echo "<li><a href='?module=bangunan&hal=1'>&laquo;</a></li>";
						}
						for ($i = $start; $i <= $end; $i++) {
							$aktif = ($i == $hal) ? "class='active'" : "";
							echo "<li $aktif><a href='?module=bangunan&hal=$i'>$i</a></li>";
						}
						if ($hal < $jmlhalaman) {
							echo "<li><a href='?module=bangunan&hal=$jmlhalaman'>&raquo;</a></li>";
						}
						?>
					</ul>
				</div>
		...
		?>
			<section class="content-header">
				<h1 class="text-center">Form Tambah Data Bangunan PKK</h1>
			</section>
			...
			<div class="box box-info" style="margin-top: 20px;">
				...
				<form class="form-horizontal" action="<?php echo $aksi; ?>?module=bangunan&act=input" method="POST" id="popup-validation" enctype="multipart/form-data">
					<div class="col-md-6">
						<div class="box-body">
							...
							<div class="form-group">
								<label for="nama_bangunan" class="col-sm-4 control-label">Nama Bangunan<span class="text-danger"> *</span></label>
								<div class="col-sm-8">
									<input type="text" class="validate[required] form-control" name="nama_bangunan" id="nama_bangunan" placeholder="Nama Bangunan">
								</div>
							</div>
					...
					<div class="col-md-12">
						<div class="box-footer">
							<a type="submit" href="appmaster.php?module=bangunan" name="cmbangunan" class="btn btn-danger"><i class="fa fa-remove"></i> Cancel</a>
