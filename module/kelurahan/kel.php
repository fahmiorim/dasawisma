...
if (empty($_SESSION['ses_user']) AND empty($_SESSION['ses_password'])){
  ...
  switch($act){
	...
	$count_query = pg_query($koneksi, "SELECT COUNT(*) as total FROM kelurahan");
	$count_result = pg_fetch_array($count_query);
	$count = $count_result['total'];
	$query_data = "SELECT * FROM kelurahan ORDER BY kode LIMIT $batas OFFSET $posisi";
          ...
          <a  class="btn bg-purple margin" data-toggle="tooltip" data-placement="top" title="Tambah" href="?module=kelurahan&act=tambahkel"><i class="fa fa-send"></i> Tambah</a>
				...
				<div class="box-body table-responsive no-padding">
                  <table id='example1' class='table table-bordered table-striped'>
					...
					<?php
					$no = $posisi + 1;
					$kel =pg_query($koneksi, $query_data);
			...
			?>
                      <tr>
						...
						<td style='text-align:center'>
							<a href="?module=editkel&id=<?php echo $kl['id']; ?>" class="btn bg-olive btn-flat margin" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fa fa-edit"></i></a>
							<a href="?module=hapuskel&id=<?php echo $kl['id']; ?>" class="btn btn-danger btn-flat margin" data-toggle="tooltip" data-placement="top" title="Hapus" onclick="return confirm('Yakin ingin menghapus data ini?')"><i class="fa fa-remove"></i></a>
						</td>
                      </tr>
					  <?php
                $no++;
						...
						if ($jml_hal > 1) {
							for ($i = 1; $i <= $jml_hal; $i++) {
								if ($i == $hal) {
									...
									echo "<li><a href='?module=kelurahan&hal=$i'>$i</a></li>";
								}
							}
						}
						?>
