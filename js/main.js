...
$(function () {
  ...
  $.tree = function (menu) {
    ...
    // Restore menu state from localStorage or based on URL parameter
    function restoreMenuState() {
      var openMenus = localStorage.getItem("openMenus");
      if (openMenus) {
        var menuArray = JSON.parse(openMenus);
        $(menu + " li.treeview").each(function () {
          var $li = $(this);
          var menuText = $li.find("span").first().text().trim();
          if (menuArray.indexOf(menuText) !== -1) {
            $li.addClass("active");
            $li.find(".treeview-menu").addClass("menu-open").show();
          }
        });
      }
    }
    ...
    function saveMenuState() {
      var openMenus = [];
      $(menu + " li.treeview.active").each(function () {
    ...
    // Restore menu state on page load with delay to ensure DOM is ready
    setTimeout(function () {
      restoreMenuState();
    }, 100);
    ...
    $(document).on("click", menu + " li a", function (e) {
      ...
      //If this is a link inside a submenu, stop propagation to prevent parent menu from closing
      if ($this.parent().parent().hasClass("treeview-menu")) {
        e.stopPropagation();
        // Save the parent menu state before navigation
        var parentTreeview = $this.parents("li.treeview");
        if (parentTreeview.length > 0) {
          var menuText = parentTreeview.find("span").first().text().trim();
          var openMenus = [menuText];
          localStorage.setItem("openMenus", JSON.stringify(openMenus));
        }
