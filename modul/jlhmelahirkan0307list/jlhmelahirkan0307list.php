...
if (empty($_SESSION['ses_user']) AND empty($_SESSION['ses_password'])){  
  ...
  switch($act){
    default:
	 $jlhmelahirkan0307list = pg_query($koneksi, "SELECT * FROM kehamilan where kodekel='0307'");
  $count=pg_num_rows($jlhmelahirkan0307list);
	echo"
	...
	<div class='box'>
        ...
        {
		?>			
		   <button class="btn bg-orange btn-flat margin" data-toggle="tooltip" data-placement="top" title="Detail"  onClick="view_records_jlhmelahirkan0307list();" ><i class="fa fa-desktop"></i> Detail</button>
		...
		<?php } ?>	
			...
			?>
					...
					<input  type="checkbox" name="chk[]" class="chk-box" value="<?php echo $lk['id'];?>"/>
						...
						<td><?php echo" $lk[jlhmelahirkan]"; ?></td>
                      </tr>
					  <?php
                $no++;
              }
