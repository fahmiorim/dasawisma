...
if (empty($_SESSION['ses_user']) AND empty($_SESSION['ses_password'])){  
  header('location:404.php');
	...
	$aksi = "modul/koperasi/aksi_koperasi.php";
	 // mengatasi variabel yang belum di definisikan (notice undefined index)
  $act = isset($_GET['act']) ? $_GET['act'] : ''; 

  switch($act){
    default:
	 $kec = pg_query($koneksi, "SELECT * FROM koperasi order by namakoperasi");
  $count=pg_num_rows($kec);
	...
	<div class='box'>
        ...
        {
		?>			 
			 <button class="btn bg-orange btn-flat margin"  data-toggle="tooltip" data-placement="top" title="lihat"  onClick="view_records_koperasi();" ><i class="fa fa-desktop"></i> Lihat</button>
		  <button class="btn bg-olive btn-flat margin" data-toggle="tooltip" data-placement="top" title="Edit"  onClick="update_records_koperasi();" ><i class="fa fa-edit"></i> Edit</button> 
		  <button class="btn btn-danger" data-toggle="tooltip" data-placement="top" title="Hapus"   onClick="delete_records_koperasi();" ><i class="fa fa-remove"></i> Hapus </button>
		  </div>
		...
		<?php } ?>	
				...
				<div class="box-body table-responsive no-padding">
                  <table id='example1' class='table table-bordered table-striped'>
					...
					<?php  
					$no = 1;
					$lingk =pg_query($koneksi, "select * from koperasi where kodekel='$_SESSION[ses_kodekel]' order by namakoperasi");
