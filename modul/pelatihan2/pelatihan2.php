...
if (empty($_SESSION['ses_user']) AND empty($_SESSION['ses_password'])){  
  ...
  $total_pages = ceil($count / $limit);
	...
	echo"
          ...
          <a  class="btn bg-purple margin" data-toggle="tooltip" data-placement="top" title="Tambah" href="?module=pelatihan2&act=tambahpelatihan2"><i class="fa fa-send"></i> Tambah</a> 
		  
		<?php
		if($count > 0)
        {
		?>			 
			 <button class="btn bg-orange btn-flat margin"  data-toggle="tooltip" data-placement="top" title="lihat"  onClick="view_records_pelatihan2();" ><i class="fa fa-desktop"></i> Lihat</button>
		  <button class="btn bg-olive btn-flat margin" data-toggle="tooltip" data-placement="top" title="Edit"  onClick="update_records_pelatihan2();" ><i class="fa fa-edit"></i> Edit</button> 
		  <button class="btn btn-danger" data-toggle="tooltip" data-placement="top" title="Hapus"   onClick="delete_records_pelatihan2();" ><i class="fa fa-remove"></i> Hapus </button>
		  </div>
		...
		<?php } ?>	
				...
				<div class="box-body table-responsive no-padding">
                  <table id='example1' class='table table-bordered table-striped'>
					...
					<?php  
					$no = $offset + 1;
					$lingk =pg_query($koneksi, "select * from pelatihan order by namapelatihan LIMIT $limit OFFSET $offset");
			...
			?>
              ...
              ?> 
                ...
                <div class='box-footer'>
                  <ul class='pagination'>
                    <?php if($page > 1): ?>
                      <li><a href='?module=pelatihan2&page=<?php echo $page - 1; ?>'>&laquo; Prev</a></li>
                    <?php endif; ?>
                    
                    <?php for($i = 1; $i <= $total_pages; $i++): ?>
                      <li class='<?php echo $i == $page ? 'active' : ''; ?>'>
                        <a href='?module=pelatihan2&page=<?php echo $i; ?>'><?php echo $i; ?></a>
                      </li>
                    <?php endfor; ?>
                    
                    <?php if($page < $total_pages): ?>
                      <li><a href='?module=pelatihan2&page=<?php echo $page + 1; ?>'>Next &raquo;</a></li>
                    <?php endif; ?>
                  </ul>
	...
	<?php		  
	 ...
	 <center><h3 class="box-title">TAMBAH DATA PELATIHAN</h3></center>
			...
			<div class="box box-info">
                ...
                <form class="form-horizontal" action="<?php echo $aksi;?>?module=pelatihan2&act=input" method="POST" id="popup-validation" enctype="multipart/form-data">
				<div class="col-md-6">
                  <div class="box-body">
					 ...
					 <div class="form-group">	
					  ...
					  <div class="col-sm-6">
                       <select class='validate[required] form-control' name='kriteria'>
						<option></option>
						<?php
									$kr = pg_query($koneksi, "SELECT * FROM kriteria order by kode LIMIT 100");
		...
		<div class="modal fade" id="myModal6" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
            <div class="modal-dialog" style="width:800px">
                    ...
                    <div class="modal-body">
                        <table id="example3" class="table table-bordered table-striped">
                            ...
                            <tbody>
								...
								$batas_modal = 10;
								$hal_modal = isset($_GET['hal_modal']) ? $_GET['hal_modal'] : 1;
								$posisi_modal = ($hal_modal - 1) * $batas_modal;

								$count_query_modal = pg_query($koneksi, "SELECT COUNT(*) as total FROM datawarga");
								$count_result_modal = pg_fetch_array($count_query_modal);
								$jmldata_modal = $count_result_modal['total'];
								$jmlhalaman_modal = ceil($jmldata_modal/$batas_modal);

								$qu = pg_query($koneksi, "SELECT * FROM datawarga order by nik LIMIT $batas_modal OFFSET $posisi_modal");
						...
						<div class="box-footer clearfix">
							<ul class="pagination pagination-sm no-margin pull-right">
								<?php
								$batas_halaman_modal = 5;
								$start_modal = max(1, $hal_modal - floor($batas_halaman_modal/2));
								$end_modal = min($jmlhalaman_modal, $start_modal + $batas_halaman_modal - 1);
								if($end_modal - $start_modal < $batas_halaman_modal - 1){
									$start_modal = max(1, $end_modal - $batas_halaman_modal + 1);
								}
								if($hal_modal > 1){
									$prev_modal = $hal_modal - 1;
									echo "<li><a href=\"?module=pelatihan2&act=tambahpelatihan2&hal_modal=1\">&laquo;</a></li>";
									echo "<li><a href=\"?module=pelatihan2&act=tambahpelatihan2&hal_modal=$prev_modal\">&lsaquo;</a></li>";
								}
								for($i=$start_modal; $i<=$end_modal; $i++){
									$aktif_modal = ($i == $hal_modal) ? "class=\"active\"" : "";
									echo "<li $aktif_modal><a href=\"?module=pelatihan2&act=tambahpelatihan2&hal_modal=$i\">$i</a></li>";
								}
								if($hal_modal < $jmlhalaman_modal){
									$next_modal = $hal_modal + 1;
									echo "<li><a href=\"?module=pelatihan2&act=tambahpelatihan2&hal_modal=$next_modal\">&rsaquo;</a></li>";
									echo "<li><a href=\"?module=pelatihan2&act=tambahpelatihan2&hal_modal=$jmlhalaman_modal\">&raquo;</a></li>";
								}
								?>
							</ul>
						</div>
				...
				<div class="col-md-12">	
                  <div class="box-footer">
                    <a type="submit"  href="appmaster.php?module=pelatihan2" name="cmpelatihan" class="btn btn-danger"><i class="fa fa-remove"></i> Cancel</a>
