...
if (empty($_SESSION['ses_user']) AND empty($_SESSION['ses_password'])){  
  ...
  switch($act){
    default:
	 $kec = pg_query($koneksi, "SELECT * FROM kriteria");
  ...
  $total_pages = ceil($count / $limit);
	...
	echo"
        ...
        {
		?>			 
			 <button class="btn bg-orange btn-flat margin"  data-toggle="tooltip" data-placement="top" title="lihat"  onClick="view_records_kriteria();" ><i class="fa fa-desktop"></i> Lihat</button>
		  <button class="btn bg-olive btn-flat margin" data-toggle="tooltip" data-placement="top" title="Edit"  onClick="update_records_kriteria();" ><i class="fa fa-edit"></i> Edit</button> 
		  <button class="btn btn-danger" data-toggle="tooltip" data-placement="top" title="Hapus"   onClick="delete_records_kriteria();" ><i class="fa fa-remove"></i> Hapus </button>
		  </div>
		...
		<?php } ?>	
				...
				<div class="box-body table-responsive no-padding">
                  <table id='example1' class='table table-bordered table-striped'>
					...
					<?php  
					$no = $offset + 1;
					$lingk =pg_query($koneksi, "select * from kriteria order by kode LIMIT $limit OFFSET $offset");
			...
			?>
              ...
              ?> 
                ...
                <div class='box-footer'>
                  <ul class='pagination'>
                    <?php if($page > 1): ?>
                      <li><a href='?module=kriteria&page=<?php echo $page - 1; ?>'>&laquo; Prev</a></li>
                    <?php endif; ?>
                    
                    <?php for($i = 1; $i <= $total_pages; $i++): ?>
                      <li class='<?php echo $i == $page ? 'active' : ''; ?>'>
                        <a href='?module=kriteria&page=<?php echo $i; ?>'><?php echo $i; ?></a>
                      </li>
                    <?php endfor; ?>
                    
                    <?php if($page < $total_pages): ?>
                      <li><a href='?module=kriteria&page=<?php echo $page + 1; ?>'>Next &raquo;</a></li>
                    <?php endif; ?>
                  </ul>
