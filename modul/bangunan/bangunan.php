...
if (empty($_SESSION['ses_user']) and empty($_SESSION['ses_password'])) {
	...
	$aksi = "modul/bangunan/aksi_bangunan.php";
	// mengatasi variabel yang belum di definisikan (notice undefined index)
	$act = isset($_GET['act']) ? $_GET['act'] : '';

	switch ($act) {
		default:
			$bangunan = pg_query($koneksi, "SELECT * FROM bangunan");
			$count = pg_num_rows($bangunan);
			echo "
	
	<div class='box'>
                <div class='box-header'>
                  <h2 class='box-title'>DATA BANGUNAN PKK</h2>";
?>
			<form method="post" name="frm">
				...
				<div style="text-align:right">
					...
					<a class="btn bg-purple margin" data-toggle="tooltip" data-placement="top" title="Tambah" href="?module=bangunan&act=tambahbangunan"><i class="fa fa-send"></i> Tambah</a>

					<?php
					if ($count > 0) {
					?>
						<button class="btn bg-orange btn-flat margin" data-toggle="tooltip" data-placement="top" title="lihat" onClick="view_records_bangunan();"><i class="fa fa-desktop"></i> Lihat</button>
						<button class="btn bg-olive btn-flat margin" data-toggle="tooltip" data-placement="top" title="Edit" onClick="update_records_bangunan();"><i class="fa fa-edit"></i> Edit</button>
						<button class="btn btn-danger" data-toggle="tooltip" data-placement="top" title="Hapus" onClick="delete_records_bangunan();"><i class="fa fa-remove"></i> Hapus </button>
				</div>
				...
				<div class="box-body table-responsive no-padding">
					<table id='example1' class='table table-bordered table-striped'>
						...
						<tbody>
							<?php
							$no = 1;
							$lingk = pg_query($koneksi, "select * from bangunan where kodekel='$_SESSION[ses_kodekel]' order by nama_bangunan");
							...
							?>
								<tr>
									...
									<td><?php echo " $lk[nama_bangunan]"; ?></td>
		...
		?>
			<center>
				<h3 class="box-title">TAMBAH DATA BANGUNAN PKK</h3>
			</center>
