...
$id = $_GET['id'];
$res = pg_query($koneksi, "select * from warung");
$r = pg_fetch_array($res);

header("Content-Type: application/vnd.ms-excel");
header("Content-Disposition: attachment; filename=Rpt_WarungKec.xls");
...
<table width="983" border="0">
  ...
  <tr>
    <td align="center"><span class="style4">LAPORAN WARUNG PKK KECAMATAN <?php echo $namakec; ?></span></td>
  </tr>
    ...
    <td>
      <table width="977" border="1" cellpadding="0" cellspacing="0" class="table table-bordered table-striped">
        <tr>
          ...
          <td width="258" align="center">Nama Warung </td>
        ...
        $no = 1;
        $tim = pg_query($koneksi, "select * from warung where kodekec='$_GET[kdkec]' order by lingkungan ");
        while ($tm = pg_fetch_array($tim)) {
        ?>
          <tr>
            <td><?php echo " $no"; ?></td>
            <td><?php echo " $tm[kelurahan]"; ?></td>
            <td><?php echo " $tm[namawarung]"; ?></td>
