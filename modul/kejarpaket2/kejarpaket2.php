...
if (empty($_SESSION['ses_user']) AND empty($_SESSION['ses_password'])){  
  header('location:404.php');
	...
	$aksi = "modul/kejarpaket2/aksi_kejarpaket2.php";
	 // mengatasi variabel yang belum di definisikan (notice undefined index)
  $act = isset($_GET['act']) ? $_GET['act'] : ''; 

  switch($act){
    default:
	 $kec = pg_query($koneksi, "SELECT * FROM kejarpaket where tahun='$_SESSION[thnaktif]'");
  $count=pg_num_rows($kec);
	echo"
	
	<div class='box'>
                <div class='box-header'>
                  <h2 class='box-title'>DATA KEJAR PAKET/KF/PAUD</h2>";
                ?>
          ...
          <a  class="btn bg-purple margin" data-toggle="tooltip" data-placement="top" title="Tambah" href="?module=kejarpaket2&act=tambahkejarpaket2"><i class="fa fa-send"></i> Tambah</a> 
		  
		<?php
		if($count > 0)
        {
		?>			 
			 <button class="btn bg-orange btn-flat margin"  data-toggle="tooltip" data-placement="top" title="lihat"  onClick="view_records_kejarpaket2();" ><i class="fa fa-desktop"></i> Lihat</button>
		  <button class="btn bg-olive btn-flat margin" data-toggle="tooltip" data-placement="top" title="Edit"  onClick="update_records_kejarpaket2();" ><i class="fa fa-edit"></i> Edit</button> 
		  <button class="btn btn-danger" data-toggle="tooltip" data-placement="top" title="Hapus"   onClick="delete_records_kejarpaket2();" ><i class="fa fa-remove"></i> Hapus </button>
		  </div>
		...
		<?php } ?>	
				...
				<div class="box-body table-responsive no-padding">
                  <table id='example1' class='table table-bordered table-striped'>
                    <thead>
                      <tr>
					   ...
					   <input type="checkbox"  name="select_all" id="select_all" value=""/>
                        ...
                        <th>No</th>
						<th>Tgl.Entry</th>
                        <th>Nama Kejar Paket/KF/PAUD</th>
						<th>Jenis</th>
						<th>Warga Belajar LK</th>
						<th>Warga Belajar PR</th>
						<th>Pengajar LK</th>
						<th>Pengajar PR</th>
					...
					<?php  
					$no = 1;
					$lingk =pg_query($koneksi, "select * from kejarpaket where tahun='$_SESSION[thnaktif]' order by namapaket");
			...
			?>
					...
					<input  type="checkbox" name="chk[]" class="chk-box" value="<?php echo $lk['id'];?>"/>
						...
						<td><?php echo" $lk[namapaket]"; ?></td>
						<td><?php echo" $lk[jenis]"; ?></td>
						<td><?php echo" $lk[waberlk]"; ?></td>
						<td><?php echo" $lk[waberpr]"; ?></td>
						<td><?php echo" $lk[pengajarlk]"; ?></td>
						<td><?php echo" $lk[pengajarpr]"; ?></td>
	...
	<?php		  
	 break;
	   case "tambahkejarpaket2":
	  
	 ?>
	 <center><h3 class="box-title">TAMBAH DATA KEJAR PAKET/KF/PAUD</h3></center>
			...
			<div class="box box-info">
                ...
                <form class="form-horizontal" action="<?php echo $aksi;?>?module=kejarpaket2&act=input" method="POST" id="popup-validation" enctype="multipart/form-data">
				<div class="col-md-6">
                  <div class="box-body">
					...
					<div class="form-group">	
					  ...
					  <div class="col-sm-8">
                        <input type="text" class="validate[required] form-control" name="namapaket" id="namapaket" placeholder="Nama Kejar Paket/KF/PAUD">
                       </div>
					 </div>
		...
		<div class="col-md-6">
             <div class="box-body">
					...
					<label for="anggotalk" class="col-sm-4 control-label">Laki-Laki<span class="text-danger"> *</span></label>
					  <div class="col-sm-3">
					    <input type="text" class="validate[required,custom[number]] form-control" name="waberlk" id="waberlk" placeholder="Laki-Laki">
                     </div>
					 </div>
					...
					<label for="waberpr" class="col-sm-4 control-label">Perempuan<span class="text-danger"> *</span></label>
					  <div class="col-sm-3">
					    <input type="text" class="validate[required,custom[number]] form-control" name="waberpr" id="waberpr" placeholder="Perempuan">
                     </div>
					 </div>
					...
					<label for="pengajarlk" class="col-sm-4 control-label">Laki-Laki<span class="text-danger"> *</span></label>
					  <div class="col-sm-3">
					    <input type="text" class="validate[required,custom[number]] form-control" name="pengajarlk" id="pengajarlk" placeholder="Laki-Laki">
                     </div>
					 </div>
					...
					<label for="pengajarpr" class="col-sm-4 control-label">Perempuan<span class="text-danger"> *</span></label>
					  <div class="col-sm-3">
					    <input type="text" class="validate[required,custom[number]] form-control" name="pengajarpr" id="pengajarpr" placeholder="Perempuan">
                     </div>
					 </div>
            ...
            <div class="modal-dialog" style="width:800px">
				...
				<div class="col-md-12">	
                  <div class="box-footer">
                    <a type="submit"  href="appmaster.php?module=kejarpaket2" name="cmwarung" class="btn btn-danger"><i class="fa fa-remove"></i> Cancel</a>
